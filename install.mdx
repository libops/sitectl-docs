...
You also need core `sitectl` 0.38.0 or newer on your system. A manual binary install does not resolve that dependency for you.
...
The current ISLE release line requires core `sitectl` 0.38.0 or newer and `sitectl-drupal` 0.10.0 or newer. Homebrew formulas are published in dependency order: core, Drupal, then ISLE.
...
The package manager will also install `sitectl` 0.38.0 or newer and `sitectl-drupal` 0.10.0 or newer as dependencies. Repository publication follows the same core, Drupal, then ISLE order so every dependency is available before the dependent package.
...
You also need core `sitectl` 0.38.0 or newer and `sitectl-drupal` 0.10.0 or newer on your system. A manual binary install does not resolve those dependencies for you.

<AccordionGroup>
  <Accordion title="Download" icon="download">

Download binaries for your system from the latest releases of [sitectl](https://github.com/libops/sitectl/releases/latest), [sitectl-drupal](https://github.com/libops/sitectl-drupal/releases/latest), and [sitectl-isle](https://github.com/libops/sitectl-isle/releases/latest). Put all three binaries on the same `$PATH`.
  </Accordion>
