...
- **Core workflow commands** stay in the core namespace and dispatch to the active context's plugin over RPC. This includes `sitectl create`, `sitectl validate`, `sitectl healthcheck`, `sitectl debug`, `sitectl set`, `sitectl converge`, and `sitectl component describe`.
...
The active context's `plugin` field determines which plugin is responsible for a given site. Commands like `sitectl debug`, `sitectl validate`, and `sitectl converge` use this to route plugin-specific work automatically.
