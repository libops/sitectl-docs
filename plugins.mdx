...
General Docker operations stay in the core CLI. Use [`sitectl compose`](/commands/compose) for context-aware Docker Compose commands and [`sitectl port-forward`](/commands/port-forward) for loopback-only access to internal services on local or remote contexts. Plugins add stack-specific value on top of that foundation: create definitions, application CLIs, API helpers, sync workflows, migrations, diagnostics, and other project-specific operations.
...
<Warning>
The released v1 compatibility set does not yet include `sitectl-isle`. Its latest published `v0.19.0` plugin remains on the legacy core v0 line while the remaining ISLE v1 components are completed.
</Warning>
...
<CardGroup cols={2}>
  ...
  <Card title="Islandora" icon="layer-group" href="/plugins/isle">
    Legacy v0 Islandora workflows from `sitectl-isle v0.19.0`. This plugin includes Drupal but is not yet compatible with core v1.
  </Card>
