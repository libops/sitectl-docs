...
An `up` with additional behavior flags, profiles, or selected services passes through unchanged so sitectl does not replace the requested operation with a plugin's full-stack start. Run reconcile explicitly first in workflows such as:
