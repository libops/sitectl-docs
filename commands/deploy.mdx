...
Dependency versions come from the reviewed checkout. In particular, ISLE uses
explicit image references in Compose rather than a shared environment tag, with
immutable digests retained where they are tracked. Use [`sitectl bump`](/commands/bump)
to propose those source changes; use `sitectl deploy` only after the resulting
branch or commit has been reviewed.
...
For Drupal and ISLE, the automatic gate does not run `drush config:import`, refresh
Search API Solr configuration, or reindex content. Import a reviewed config-sync
change explicitly with `sitectl drupal drush cim -y`. When Solr or the Search API
Solr module changed, run `sitectl drupal solr-config refresh --reindex`; reindexing
only runs when that command changes the runtime Solr configuration. After changing
the MariaDB image, run `sitectl mariadb upgrade`; an already-current database is a
successful no-op. Keep the pre-deploy backup until these follow-ups and application
checks pass.
