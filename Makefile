...
# from replacing that graph, and -mod=readonly prevents generation from changing
# its dependency files. The generator runs from inside its module dir and is
# passed the docs root as the output base.
docs-snippets:
	cd scripts/gen-docs-snippets && GOWORK=off go run -mod=readonly . "$(CURDIR)"

docs-snippets-check: snippet-dependencies-check docs-snippets
	git diff --exit-code -- \
		scripts/gen-docs-snippets/go.mod \
		scripts/gen-docs-snippets/go.sum \
		snippets/commands
