...
1. Tag and publish core `sitectl` `v0.38.0`.
2. In each plugin, run `go get github.com/libops/sitectl@v0.38.0` followed by `go mod tidy`, ensure integration tests install stable core v0.38.0 or newer, then run the full plugin tests without a local `go.work` override.
3. Release `sitectl-drupal` before `sitectl-isle`, then release the other application plugins. ISLE packages require `sitectl-drupal >= 0.10.0` as well as `sitectl >= 0.38.0`, so the publish and install dependency order is core, Drupal, then ISLE.
4. Publish or update the Homebrew core formula before plugin formulas. Homebrew dependencies cannot express the same minimum version reliably, so ordering is the compatibility gate there.
