...
Plugin executables embed the sitectl SDK version selected by their `go.mod`; installing a newer core binary does not update SDK behavior inside an already-built plugin. Releases that depend on the pull/build preparation boundary, app-only migration start, or non-PHP ingress runtime contract therefore use `v0.38.0` as their first compatible core version.
...
3. Release `sitectl-drupal` and wait for all publication jobs before releasing `sitectl-isle`; then release the other application plugins. ISLE packages require `sitectl-drupal >= 0.10.0` as well as `sitectl >= 0.38.0`, so the publish and install dependency order is core, Drupal, then ISLE. Homebrew cannot express the same minimum versions reliably, making this ordering the compatibility gate there.
...
5. Update `sitectl-docs` last. Record each published version and exact tag commit in `scripts/snippet-dependencies.json`, regenerate the snippets from those commits, and merge only after the manifest checker proves every tag resolves to its recorded SHA.

Plugin GoReleaser configurations run `scripts/check-sitectl-core-version.sh` and refuse a release built with an older or prerelease SDK. Debian, RPM, and APK packages also declare their format-specific `sitectl >= 0.38.0` dependency. Renovate can propose later module upgrades after tags exist, but the initial core-first bump is an intentional release step.
