...
3. Release independent application plugins after core, and wait for each publication workflow before updating template smoke tests to the new package version. Release an including plugin only after every included plugin is available. `sitectl-isle` is currently excluded from the v1 release set until its remaining component work is complete; do not publish it merely because Drupal v1 exists.
