...
This builds and installs `sitectl`, discovers the supported sibling application
plugin checkouts, and runs `make install` in each checkout that is present. Run
`make work` first to create one local Go workspace for core and those plugins.
Normal build and test targets do not rewrite dependency files; use the explicit
dependency-update target when changing module versions.
