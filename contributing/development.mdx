...
Plugin executables embed the sitectl SDK version selected by their `go.mod`; installing a newer core binary does not update SDK behavior inside an already-built plugin. Releases that depend on the current rollout and service-component fixes therefore use `v0.37.0` as their first compatible core version.

Use this order for any release that raises the minimum core SDK version:

1. Tag and publish core `sitectl` `v0.37.0`.
2. In each plugin, run `go get github.com/libops/sitectl@v0.37.0` followed by `go mod tidy`, ensure integration tests install stable core v0.37.0 or newer, then run the full plugin tests without a local `go.work` override.
3. Release `sitectl-drupal` before `sitectl-isle`, then release the application plugins. The v0.37 compatibility release makes ISLE packages require `sitectl-drupal >= 0.10.0` as well as `sitectl >= 0.37.0`.
