...
Plugins expose create definitions through `plugin.metadata`. A create definition describes a first-class Compose template: repository, branch, lifecycle commands, init artifacts, image specs, and the default definition for the plugin.

Core sitectl consumes create definitions in several places:

- `sitectl create list` and `sitectl create <plugin>/<definition>` for guided stack creation
- `sitectl image set` and create-time image flags for local Compose image and build-arg overrides
- `sitectl compose up` for local first-run reconcile on plugin-owned contexts

The same metadata should therefore be deterministic and safe to reuse. Lifecycle command output can be human-facing when run directly, but plugin RPC stdout rules still apply: logs, prompts, and progress from RPC handlers belong on stderr, while stdout is reserved for the response envelope.

See [Compose reconcile contract](/contributing/compose-reconcile) for the local `compose up` contract and [Application plugin template](/contributing/app-template) for the thin app-plugin scaffold.
...
|---|---|---|
| `plugin.metadata` | Returns plugin name, version, description, includes, create definitions, deploy definitions, image specs, and capability flags. | `plugin.NewSDK(metadata)` |
...
Registered by `sdk.RegisterValidateRunner(runner)`. Core `sitectl validate` runs its own core validators first, then invokes `validate.run` if the context plugin supports validation (`CanValidate: true`). The plugin returns `[]validate.Result` in the JSON response `result`. Core merges those results with its own before rendering the final report.
...
Unlike `validate` and `healthcheck`, core `verify` has no built-in behavioral checks. In interactive mode, a context with no plugin verifier renders a warning for operator review. In release automation, `--strict` turns missing, empty, or warning results into failures. `--format` and `--strict` are host-owned because core renders and evaluates the final report; all other unclaimed flags are passed through in `args` for the plugin's verify runner.
...
Create definitions are also used outside `create.run`: core reads them from metadata for `sitectl create list`, local image override handling, and command-scoped `sitectl compose up` reconcile.
...
| `sitectl stats` | `ingress.routes` | Core reports context, ingress, ports, and Docker stats; plugin supplies app-owned route descriptors |
