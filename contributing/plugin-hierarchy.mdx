...
When a full plugin inspection is needed (e.g. for `sitectl converge` or `sitectl validate`), sitectl runs `sitectl-<plugin> __sitectl-rpc` with a JSON request envelope for the `plugin.metadata` method. The plugin returns a JSON response envelope with its capabilities: name, version, description, included plugins, and boolean flags for which SDK runner interfaces it has registered (`CanDebug`, `CanConverge`, `CanSet`, `CanValidate`, `CanDeploy`, `CanCreate`).
