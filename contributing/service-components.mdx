...
The goal is to keep common services boring and portable. A service like MariaDB or Solr should not be reimplemented separately for Drupal, ISLE, WordPress, Omeka, OJS, and ArchivesSpace. The shared service operation belongs in core sitectl; the application owns only the wiring that makes that service useful inside that application.
...
Core ingress component support must cover:

- `http` for plain local or internal HTTP
- `https` / `https-cloudflare-origin` for Cloudflare Origin CA certificates mounted at `certs/cert.pem` and `certs/privkey.pem`
- `https-letsencrypt` for ACME-managed certificates when the hostname resolves directly to the origin
- `https-custom` for an operator-supplied certificate and private key pair mounted at the same paths
- `https-mkcert` for mkcert-generated development certificates
- domain and ACME email values stored directly in Compose and dynamic Traefik config
- trusted upstream proxy IPs and upload/read timeout tuning

Application Compose projects may add route files, labels, service dependencies, or app-specific dynamic config references, but app plugins should not each invent their own TLS, domain, trusted proxy, or upload-limit implementation. Otherwise, every application drifts into a different ingress model.
...
Triplet is application-scale behavior inside ISLE. The ISLE plugin owns its `iiif` and `iiif-topology` component wiring, while core Compose commands own runtime inspection; there is no separate thin Triplet lifecycle plugin.
