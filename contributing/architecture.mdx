...
Reusable infrastructure services are intentionally owned by their own plugin/template repos, not by sitectl core and not by each application plugin. For example, Traefik, Solr, MariaDB, Postgres, Valkey, and Triplet should each have one canonical compose definition in their own service repo.

Applications merge those service definitions when a component is enabled and apply only their target-specific wiring, such as dependencies, environment variables, or app-specific volume mounts. This keeps standalone deployment and embedded deployment aligned without maintaining two compose files for the same service.
...
|---|---|---|---|
| `DebugHandler` | `RegisterDebugHandler` | `debug.run` | `sitectl debug` |
...
`sitectl validate` is distinct from the others: core runs its own validators first, then invokes the plugin's `validate.run` method, decodes the returned structured results, and merges everything before rendering. The plugin does not render; it just returns structured data.
