...
- The caller's repository-scoped `GITHUB_TOKEN` creates the GitHub release and uploads its artifacts.
- The `HOMEBREW_REPO` secret is exposed to GoReleaser only as `HOMEBREW_REPO_TOKEN` and may update the separate `libops/homebrew` tap. Do not replace `GITHUB_TOKEN` with this cross-repository token.

The application plugin template leaves package-repository publication disabled with `publish-package-repo: false`. A derived plugin should enable it only after its repository has a trusted package publisher and the required cloud identity and variables. Leaving it disabled still publishes GitHub release artifacts and the native package files attached to the release. Homebrew publication is independent and runs only when the derived repository points at a tap it controls and provides `HOMEBREW_REPO`.
...
- caller workflow: [sitectl `goreleaser.yaml` at the documented v0.39.0 commit](https://github.com/libops/sitectl/blob/90c341271b38c998261efff6d81dc33217c747f7/.github/workflows/goreleaser.yaml)
- pinned reusable workflow: [`reusable-goreleaser.yaml` at `481df511`](https://github.com/libops/terraform-linux-packages/blob/481df51116aed2efd1c002ef1ef6a287699828a0/.github/workflows/reusable-goreleaser.yaml)
- pinned package publisher used by that workflow: [`publish-package-repo.sh` at `72d2f0c3`](https://github.com/libops/terraform-linux-packages/blob/72d2f0c3b01e5e396d6db074108b95e95eedf4d4/scripts/publish-package-repo.sh)

Required GitHub secrets:

- `HOMEBREW_REPO`: cross-repository token used only to update the Homebrew tap
