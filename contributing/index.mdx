...
The current local development checkout set in this workspace is:

- `sitectl`: core binary; owns the operator-facing commands
- `sitectl-archivesspace`: ArchivesSpace application operations
- `sitectl-drupal`: Drupal operations; also included by the ISLE plugin
- `sitectl-isle`: ISLE plugin; stack logic, component definitions, ISLE-specific jobs
- `sitectl-ojs`: Open Journal Systems application operations
- `sitectl-omeka-classic`: Omeka Classic application operations
- `sitectl-omeka-s`: Omeka S application operations
- `sitectl-wp`: WordPress application operations
...
2. Discover supported sibling checkouts named `../sitectl-<application>`
3. Build and install every supported plugin checkout that is present

Run `make work` first to create a local Go workspace containing core and all
supported sibling plugin checkouts. The core `make test` and `make build`
targets download declared dependencies but never rewrite `go.mod` or `go.sum`;
dependency upgrades use the explicit `make deps-update` target.

If you are only working on a single plugin:

```bash
cd ../sitectl-isle && make install
cd ../sitectl-archivesspace && make install
```
...
- application behavior in any supported `sitectl-<application>` plugin
- included-plugin behavior such as Drupal extensions used by ISLE
...
- core v1.0.2 recovery caller workflow: [sitectl `goreleaser.yaml` at `589a634cae78ad7b02d70cbae054e9b17e649669`](https://github.com/libops/sitectl/blob/589a634cae78ad7b02d70cbae054e9b17e649669/.github/workflows/goreleaser.yaml)
- core v1.0.2, Drupal v1.0.0, and LibOps v1.4.0 shared workflow: [`sitectl-plugin-goreleaser.yaml` at `e1e30b58c9c566f72b22f03e637cd5218d635727`](https://github.com/libops/.github/blob/e1e30b58c9c566f72b22f03e637cd5218d635727/.github/workflows/sitectl-plugin-goreleaser.yaml)
- package publisher pinned by that shared workflow: [`terraform-linux-packages` at `b14fd2f95b4017e897c595b3321b9cc3f48b5ddd`](https://github.com/libops/terraform-linux-packages/tree/b14fd2f95b4017e897c595b3321b9cc3f48b5ddd)
...
- package publisher pinned by that five-application workflow revision: [`terraform-linux-packages` at `285183831e6e5c14ec34d87e208629949f5b627c`](https://github.com/libops/terraform-linux-packages/tree/285183831e6e5c14ec34d87e208629949f5b627c)
