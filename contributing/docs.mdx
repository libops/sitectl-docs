...
Command reference blocks (the auto-generated usage and flags tables) live in `snippets/commands/` and are generated from the Go source. Do not edit them by hand because they will be overwritten on the next run.
...
`sitectl-wp`, and `sitectl-triplet`. It renders each command to an `.mdx` snippet
file.
