...
Command reference blocks (the auto-generated usage and flags tables) live in `snippets/commands/` and are generated from the Go source. Do not edit active generated snippets by hand because they will be overwritten on the next run. Regeneration removes obsolete `.mdx` files that carry the active auto-generated header while preserving manually maintained files and explicitly labeled legacy snapshots.
...
Pull requests run `make docs-snippets-check`, which regenerates the active files from the exact core and plugin commits recorded in `scripts/snippet-dependencies.json` and fails when the result differs from the committed snippets. The dependency check requires an exact manifest/module/replace set and verifies that every active and legacy stable release tag resolves to its recorded commit. The same required workflow runs the pinned Mintlify CLI through `make docs-check` to validate the strict documentation build and internal links.
...
tree plus every plugin in the active v1 compatibility set: `sitectl-drupal`,
...
file. `sitectl-isle` remains on its legacy v0 line, so it is recorded in
`scripts/legacy-snippet-dependencies.json` and its four direct-command snippets are
explicitly labeled legacy snapshots rather than being compiled against the v1 SDK.
...
compile. Move its reviewed release record to a separately validated legacy manifest,
remove it from the active module and hosted checkout graph, and label any retained
reference snippets with the exact legacy release.
