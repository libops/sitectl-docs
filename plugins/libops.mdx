...
```bash
brew install libops/homebrew/sitectl-libops
# or, after configuring packages.libops.io
sudo apt install sitectl-libops
```

The reviewed managed-platform set pairs `sitectl-libops v1.4.0` with core `sitectl v1.0.2` and RPC schema 1. Manual binary installs must keep both binaries aligned; verify with `sitectl --version`, `sitectl-libops --version`, and `sitectl libops --help`.
