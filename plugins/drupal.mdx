...
Use ordinary git and Composer workflows inside the project checkout for extension branches, patches, and upstream source changes.

## Deploy and Database Updates

The downstream checkout owns Drupal through its Composer manifests and lockfile. `sitectl deploy` pulls dependencies and rebuilds the local image while the current stack is still online. After stopping it, deploy starts only the Drupal service and its required Compose dependencies, waits for Drupal's installation marker, then runs `drush updb -y` and `drush cr`. Database-update and cache-rebuild failures stop the deploy; only a successful migration is followed by a bounded full-stack start and health wait.

Back up the database and the public/private files volumes before changing Drupal core or contributed dependencies. Commit Composer and component-generated changes in the same update branch so another checkout can reproduce the image and configuration.
