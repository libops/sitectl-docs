...
The downstream checkout owns Drupal through its Composer manifests and lockfile. `sitectl deploy` pulls dependencies, rebuilds the local image, waits for Drupal, then runs `drush updb -y` and `drush cr` before a final bounded health wait. Database-update and cache-rebuild failures stop the deploy.
