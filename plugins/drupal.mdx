---
title: Drupal plugin
description: Drupal-oriented sitectl workflows including Composer, Drush, Crosswalk metadata profiles, login links, and database and config sync.
---
...
Capture the selected site's active Drupal model as an editable Crosswalk profile draft, or run Crosswalk with the context's resolved JSON:API endpoint:
...
Crosswalk owns the mappings and duplicate-detection policy; sitectl owns live Drupal configuration acquisition and endpoint resolution. See the [Crosswalk workflow](/plugins/drupal/crosswalk) for profile review, institution-specific identifiers, credentials, and deployment guidance.
