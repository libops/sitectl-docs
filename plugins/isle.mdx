...
description: Islandora sitectl workflows including guided site creation, component management, Workbench operations, Fedora and Blazegraph sync, and migration utilities.
---
...
Create a full-state, checksummed recovery bundle for the active context:

```bash
sitectl isle recovery plan
sitectl isle recovery backup --output /var/backups/isle/site-$(date +%F).tar.gz
sitectl isle recovery validate --input /var/backups/isle/site-2026-08-08.tar.gz
```

See [Full-state recovery](/plugins/isle/recovery) for the authoritative-state boundary, off-host storage requirements, destructive restore workflow, and recovery-drill gate.

## Crosswalk Workbench operations

Preflight media paths, reconcile profile-bound Crosswalk artifacts after node creation, retry only recognized media-upload timeout cascades, and roll back an exact Workbench create artifact with guarded context jobs:
...
See [Crosswalk Workbench operations](/plugins/isle/workbench) for the artifact trust boundary, all four jobs, and rollback safety requirements.
...
sitectl drupal crosswalk profile create repository-items --bundle islandora_object
```

See [Crosswalk](/plugins/drupal/crosswalk) for the site-bound profile and JSON:API endpoint workflow.
