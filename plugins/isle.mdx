...
The Islandora plugin adds Islandora-specific behavior on top of the core sitectl workflow. The plugin binary is `sitectl-isle`. Direct plugin commands live under `sitectl isle` for ISLE-only operations such as cache warming, migration helpers, and fcrepo sync. Create, validate, debug, healthcheck, set, converge, and component inspection are core sitectl commands that dispatch to the Islandora plugin through the active context.
