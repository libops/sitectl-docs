...
<Warning>
This page describes the legacy `sitectl-isle v0.19.0` line with core `sitectl v0.40.0` and `sitectl-drupal v0.12.0`. ISLE is not part of the sitectl v1 compatibility set and still needs its remaining v1 component work. Do not run this plugin with core v1.
</Warning>
...
<Steps>
  <Step title="Clone the template">
    The legacy `v0.19.0` plugin clones the `libops/isle` `v1.0.0` template into your chosen directory. Pass `--template-repo` and `--template-branch` to use a different source.
  </Step>
