...
For OJS, Omeka, and ArchivesSpace, the published LibOps base-image tag names the upstream application release and the downstream template image adds local customizations. Drupal, ISLE, and WordPress keep Composer-owned application dependencies in the downstream checkout. The plugin create metadata mirrors that ownership so `compose up` and `deploy` rebuild the correct layer. Mixing a bundled application tree with a second Composer-owned copy is unsupported because neither layer would have unambiguous ownership.
