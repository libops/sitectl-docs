...
TLS mode selection, domain publication, Let's Encrypt settings, trusted upstream proxies, and upload/read timeout tuning belong to the `ingress` component. App plugins supply app-specific router names, route files, and service environment values so a single component update can keep Traefik and the application aligned.
