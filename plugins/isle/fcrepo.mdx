...
The `fcrepo` component controls whether the Fedora service, its volumes, and related Islandora configuration are part of your stack. The LibOps template defaults to `superseded` with Drupal private files; enable Fedora only when the site requires the Fedora-backed repository.
...
| `superseded` | Remove Fedora because binaries are stored through another filesystem or storage backend. |
...
```bash
sitectl set fcrepo superseded --isle-file-system-uri public
```
