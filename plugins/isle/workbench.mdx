...
Run `sitectl job list` against an Islandora context to confirm that the jobs are available.

<Note>
  Every job path is an exact, canonical, absolute path on the selected context host. For a remote context, it names a path on the remote server, not on the workstation running sitectl. Root paths, relative paths, URLs, path normalization such as `/tmp/../file.csv`, and symlinked trust boundaries are rejected.
</Note>

## Provision the artifact contract

Crosswalk writes `crosswalk-artifacts.json` beside each Workbench CSV. The manifest binds the batch to its transformation specification, optional profile and model fingerprints, transformation policy, and each CSV's byte count, SHA-256 digest, and data-row count.
