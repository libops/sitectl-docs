...
<Steps>
  ...
  <Step title="Verify site behavior">
    After healthcheck succeeds, use `sitectl verify` to run deeper application checks supplied by the active plugin. Use strict mode in CI so a missing, empty, or warning-only verifier cannot pass a release gate.
